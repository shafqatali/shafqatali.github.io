...
function updateStandardContent(hook, videoclass, tableclass) {
  if (isLiveSite) {
    ...
    for (var z = 0; z < nestedItems; z++) {
      var listItems = Array.from(document.querySelectorAll("".concat(hook, " ol:not(.pagination):not(.wrapped) > li, ").concat(hook, " ul:not(.service-team):not(.pagination):not(.wrapped) > li")));
