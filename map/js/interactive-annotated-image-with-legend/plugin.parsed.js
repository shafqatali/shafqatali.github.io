...
function drawPins(wrapperId, counter, filter, criteria) {
  ...
  if (filter) {
    pins = locationsList.filter(function (data) {
...
function fetchPinData(wrapperId, dCity, dType) {
  ...
  var type = locationsList.filter(function (data) {
    return data.id == dType;
  });
  var cities = type[0].cities; //get required city form JSON Object

  var city = cities.filter(function (data) {
...
function scrolledToDetails(wrapperId) {
  setTimeout(function () {
    $('html, body').stop().animate({
      scrollTop: $('#' + wrapperId + ' .pin-details').offset().top
    }, 500);
  }, 500);
...
function screen_change_interactive_annotated_image_with_legend() {//do nothing for now
